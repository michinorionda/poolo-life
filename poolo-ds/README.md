...
**正規の基準ページ（ページ構成の型・2026-09-01〜）**: `../event-tomonoura/index.html` … 新しい告知ページはこの並びを出発点にする（`BRAND.md` 4章）
...
| `.og-wrap.og--photo` | 写真を敷く（左にコピー・右に写真） | 現地開催の回。文字は白パネルに退避 |
| `.og-wrap.og--stack` | 写真を上に、コピーを中央に | **申込導線がLINEの回**。構成要素は `.og-photo` / `.og-body`（`.og-eyebrow` ＋ `.og-title`）/ `.og-band`（`.og-band-when` ＋ `.og-band-meta`） |
| `.og-wrap.is-fluid` | 可変（編集型サムネ） | 告知ページに直接埋め込む。書き出し不要 |

**LINEで配る回は `.og--stack` を使う。** LINEはトークでOG画像を正方形に近い形（中央 630×630 あたり）で切ることがあり、横位置の組み（`.og--photo` のように左にコピー・右に写真）だとタイトルの頭が欠ける。2026-09-08 にゼミLPで実際に起きて、「学びの場を創る。」が「場を創る。」まで欠けた。`.og--stack` は主要素をすべて中央に寄せてあるので、切られても文意が残る。
書き出す前に、中央 630×630 の内側に主要素が収まっているかを確かめること（`event-30s-v01-260910/og-source.html` に、その目安を破線で描いた台がある）。
...
| `.hero-eyebrow` / `.hero-title`(.t-sm[.is-strong|.is-long]/.t-lg[.is-long] .em/.num) / `.hero-lead` / `.hero-foot` | Hero内のコピー要素（見出しは明朝、強調語だけブルー）。`.num` は見出しの中の数字を図として立たせる（「30代」「90分」）。**書体は変えず**、1.32em ＋ 字間 −0.02em で差をつける。一度 Jost にしていたが、Jost は 0 が真円で抑揚がゼロ、明朝は縦画が太く横画が細いので、同じ行に並べると数字だけ monoline になり値札を貼ったように見えた（2026-09-08 に差し戻し）。**BRAND.md の「欧文・数字は Jost」は、日付帯やラベルのように欧文が単独で立つ場所の話**で、明朝の見出しの内側には持ち込まない。**色も足さない**（ブルーは `.em` ひとつに任せる。同じ行に色が2つ入ると主役が決まらない）。比較は `../event-taikenkai-2609/num-compare.html`。OG画像側の対応クラスは `slides.css` の `.og-wrap .num`（`.og-title` だけでなく `.og-sub` / `.og-eyebrow` の中でも使える）。<br>**`.t-sm.is-long`** は小さい行に長い和文（14文字以上が目安）が入る回に付ける。既定の `clamp(16px, 1.9vw, 24px)` は版面が `--shell-max` で頭打ちになったあとも 1.9vw のまま伸びるので、**広い画面ほど1行に入らなくなる**（1160px で収まって 1400px で折り返す、という逆転が起きる）。上限で頭打ちを先に来させる（字間を .10em に締めたぶん、24px まで戻しても1行に収まる。長い行では級数より字間のほうが効く）。<br>**`.t-sm.is-strong`** は小さい行に存在感を持たせる回に付ける（34px ／ 字間 .10em）。既定の 24px ＋ 字間 .24em は「ラベルの字間」で、小さいから成立している。**級数と字間はセットで動かす**（大きいまま .24em を残すと間延びして逆に弱く見える）。太さは 700 のまま。900 まで上げると2行が競って主従が消える。比較は `../event-taikenkai-2609/tsm-compare.html`。<br>**`.t-lg.is-long`** は大きい行が `.hero--long-title` でも収まらない回に付ける（8文字以上が目安）。t-lg は `.nb` で折り返しを止めることが多く、収まらないと版面の右padding を越えて**写真に接する**（切れないので気づきにくい）| Hero |
...
| `.gallery` / `.gallery-head`(-text／-en と -rule は英字撤去のため不使用) / `.g-grid` / `.g-item`(.is-wide/.is-tall/.is-big) / `.g-strip` / `.gallery-note` | 写真ギャラリー（点数の多い旅ページ用。モザイク or 全幅の横スクロール帯。目地は1pxヘアラインのみ） | Content |
